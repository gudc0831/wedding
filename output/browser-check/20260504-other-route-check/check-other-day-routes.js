async (page) => {
  ...
  const ids = ["day2", "day3", "day4", "day5", "day6"];
  const results = [];

  for (const id of ids) {
    const card = page.locator(`[data-route-map-card="${id}"]`);
    await card.locator("[data-route-map-open]").click();
    await page.waitForTimeout(7000);

    const path = `D:/Wedding/output/browser-check/20260504-other-route-check/${id}-route-check.png`;
