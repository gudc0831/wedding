...
체코·스위스·이탈리아 허니문 가이드를 정적 HTML로 배포하기 위한 저장소입니다.

## 주요 파일

- `index.html`: GitHub Pages 루트 주소에서 체코 / 스위스 / 이탈리아 가이드를 선택하는 진입점
- `czech_honeymoon_guide.html`: 2026년 7월 11일-15일 체코 첫 구간과 7월 26일-27일 귀국 전 체코 구간을 분리한 전용 페이지
- `switzerland_honeymoon_guide.html`: 2026년 7월 16일-21일 스위스 일정 전용 페이지
...
GitHub Pages deployment also runs a browser smoke test after publishing. It opens each route-map country page, clicks each `지도 열기`, and fails the workflow if the page renders an iframe fallback, a Google account/billing overlay, a configuration error, or unexpected route fallback text instead of the Maps JavaScript API route map.
